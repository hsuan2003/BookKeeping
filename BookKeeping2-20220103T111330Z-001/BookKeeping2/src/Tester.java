...
public class Tester {
	...
	static Meal meals = new Meal(6000, "meal", 0);
	static Others others= new Others(1500, "others", 0);
	static int check;
	...
	public static void bookKeeping(String name, int price) {
		...
		if(check == 1) {
			...
			System.out.print("類別： 1.正餐 2.其他: ");
			int catagory = sc.nextInt();
			if(catagory == 1) {
				meals.addMeals(name,price);
			}else {
				others.addOthers(name,price);	
			}
		}else if(check == 2) {
	...
	public static void detail() {
		if(check == 1) {
			...
			System.out.print(meals.detail());
			System.out.print(others.detail());
		}else if(check == 2) {
